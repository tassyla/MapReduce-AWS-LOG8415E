...
def build_benchmark_user_data() -> str:
    return dedent(
        ...
        openjdk-11-jre-headless tar aria2 openssh-server python3-pip ca-certificates
    ...
    # Start parallel downloads (CDN first) with multi-connection
...
ENV
        ...
        # Validate archives; if invalid or missing, retry once from the Apache archive
        if ! tar -tzf hadoop-3.3.6.tar.gz >/dev/null 2>&1; then
          rm -f hadoop-3.3.6.tar.gz || true
          aria2c --allow-overwrite=true -x 16 -s 16 -k 1M -o hadoop-3.3.6.tar.gz \
            https://archive.apache.org/dist/hadoop/common/hadoop-3.3.6/hadoop-3.3.6.tar.gz || true
        fi

        if ! tar -tzf spark-3.5.1-bin-hadoop3.tgz >/dev/null 2>&1; then
          rm -f spark-3.5.1-bin-hadoop3.tgz || true
          aria2c --allow-overwrite=true -x 16 -s 16 -k 1M -o spark-3.5.1-bin-hadoop3.tgz \
            https://archive.apache.org/dist/spark/spark-3.5.1/spark-3.5.1-bin-hadoop3.tgz || true
        fi
...
UNIT
        ...
        # Wait up for HDFS to become available; try starting via script if needed
